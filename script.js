...
fetch(URL)
  ...
  .then((jsObject) => {
    ...
    for (let i = 0; i < business.length; i++) {
      ...
      image.src = business[i];
      image.alt = business[i].name;
      ...
      document.querySelector("div.cards").appendChild(address);

      // Include a business Description
      let description = document.createElement("p");
      description.textContent = business[i].description;
      card.appendChild(description);
      document.querySelector("div.cards").appendChild(description);
    }
  });
